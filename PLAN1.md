...
## 依存関係
- `extract_exif_worker.mjs` - 別プロセスでEXIF抽出を行うWorker
...
### 2. プールの初期化
- Git reader pool: CONCURRENCY = 8 の `git cat-file --batch` プロセス
- EXIF worker pool: EXIF_WORKER_COUNT = 4 の子プロセス
...
### 7. EXIF抽出 (Workerプロセス)
- extract_exif_worker.mjs にIPCでバイナリを渡す
- 別プロセスでEXIF抽出を実行
- 失敗時はfalseが返る
...
### 9. 並列処理 (2段構成)
- 第1段: Git reader pool (CONCURRENCY=8) - git cat-file --batch プロセス
- 第2段: EXIF worker pool (EXIF_WORKER_COUNT=4) - 子プロセス
...
extract_exif_worker.mjs              - EXIF抽出Worker (子プロセス)
```

## 実装メモ
- Git reader pool: 複数の `git cat-file --batch` プロセスをプール化し、ロック机制で排他アクセス
- EXIF worker pool: 複数の子プロセスをIPCで管理し、EXIF抽出を並列実行
